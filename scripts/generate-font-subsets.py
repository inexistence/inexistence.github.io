...
WEIGHTS = (400, 500, 700)
CHUNK_COUNT = 48
GENERATOR_VERSION = "2026-07-24.1"
...
def build_static(fonts: dict[int, Path]) -> None:
    ...
    for weight, source in fonts.items():
        ...
        subset_font(source, characters & available, STATIC_DIRECTORY / f"noto-sans-sc-static-{weight}.woff2")
...
def build_comment(fonts: dict[int, Path]) -> None:
    ...
    for weight, source in fonts.items():
        for index, codepoints in enumerate(chunks(font_codepoints(source)), start=1):
            ...
            subset_font(source, codepoints, COMMENT_DIRECTORY / filename)
            rules.extend([
                "@font-face {",

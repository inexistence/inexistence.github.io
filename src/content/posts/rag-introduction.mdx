...
import RagRetrievalLab from '../../components/RagRetrievalLab.tsx';
import RagPipelineLab from '../../components/RagPipelineLab.tsx';
import '../../styles/rag-labs.css';
...
下面用一个很小的离线语料试试看。它不会调用任何模型，但能把两种检索的差别放到同一句问题上：关键词究竟在找什么，语义检索又保留了什么。

<RagRetrievalLab client:visible />

当问题里的字面词没有出现在资料里，关键词检索可能无从下手；而一组经过训练的向量，会把“我不想要了”和“退货与退款”放到相近的方向。真实系统中的向量维度、语料和模型都远比这里复杂，但排序的基本思路相同。
...
在真实系统里，这些步骤会彼此影响：分块边界决定了能否带上完整条件，召回数量决定了上下文里会混入多少信息，重排序则决定哪些片段真正交给问答模型。下面用一条固定的预订规则把这条链路走一遍。

<RagPipelineLab client:visible />

注意这里的最后一步是“带来源的提取式回答”，而不是让模型自由补全文本。它用来说明：RAG 的回答质量始终受进入上下文的证据约束；如果资料不够，正确的系统应该承认不知道。

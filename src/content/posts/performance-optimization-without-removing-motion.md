---
title: "如何给 Astro 博客做一次性能优化"
description: "从 Lighthouse 审计出发，记录响应式图片、按需组件样式、Waline 延迟加载与 Astro 页面切换中的一次完整性能优化，以及其中几个比“拿分”更重要的修复。"
...
报告还帮我排除了一个看似显眼、却不该立刻动手的方向：当时真实的 LCP 是文本区域，不是封面或首页狸猫插画。也就是说，盲目给图片加 `fetchpriority="high"` 未必会让 LCP 变好，反而可能和关键字体、CSS、正文竞争带宽。

下面按 Lighthouse 报告提示的顺序，记录这次如何逐项判断：哪些应该修、哪些需要换一种实现、哪些在当前页面不该为了分数而动。
...
| LCP 图片优先级 | 当前 LCP 不是图片 | 暂不添加，等真实 LCP 改变后再测量 |
...
- 首页的入场动画和漂浮动画保留；
- `ClientRouter` 保留；
- 不因为“可能是 LCP”就给每张首屏图片加 `fetchpriority="high"`；
- 外链图片不在构建阶段改写；
...
组件库不再引入全量样式，而是保留本站实际使用组件的样式；页面原有的 Card、Tag、Button 等视觉效果不变。Waline 则延后到评论区即将进入视口时，再加载客户端、基础样式和评论字体。这样首屏不必为页面底部的交互付出下载成本，用户滚到评论区时仍能直接留言。

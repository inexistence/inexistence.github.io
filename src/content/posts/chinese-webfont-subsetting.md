...
个人博客用中文字体，目标其实很朴素：跨设备看起来像同一套字，又别让访客为三份完整 Noto Sans SC 买单。完整简体源字体每个字重大约一兆多，三个字重加起来接近 3.5 MB——对 GitHub Pages 上的静态站来说，这是明显的首屏负担。
...
| 加载 | 几乎每页都可能用到 | 仅文章页 / 留言板等挂了 Waline 的页面 |
| 产物 | 本地生成，不提交 | 提交进仓库 |
...
优先级表不依赖文章内容，所以发新文不会打乱评论分块，也不会让访客缓存的评论字体无故失效。浏览器只请求当前评论区实际用到的块。

分类页、首页等没有 Waline 的页面，不会加载评论 CSS。
...
4. **验证层是否加码**  
   目前全量哈希在 ensure；verify 做存在性与接线检查。若出现绕过 ensure 仍进部署的路径，再考虑在 verify 里对 fragment 做抽样或全量哈希——在那之前，重复计算多半只是拖慢本地反馈。
...
日常开发继续 `npm run dev` / `npm run build` 即可。改评论字体策略或源字体之后，本地更新 comment 产物与 `manifest-fragment.json` 并提交；静态子集仍交给缓存与生成器。
